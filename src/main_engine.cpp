...
#define SOLINOID_PIN 6

unsigned long startFire = -9999;

void commands(String message){
  if(message == "Fire"){
    startFire = millis();
    Serial.print("OPENING SOLINOID");
  }
}
...
void loop(){
    console.run();
    if (millis() - startFire < 5000){
      digitalWrite(SOLINOID_PIN, HIGH);
    } else {
      digitalWrite(SOLINOID_PIN, LOW);
    }
}